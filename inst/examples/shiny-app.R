# A Shiny app exercising every basecoat input as a Shiny input, including the
# four that need bc_shiny_deps(): bc_radio_group(), bc_slider(), bc_select()
# and bc_combobox(). The rest, bc_checkbox(), bc_switch(), bc_input(),
# bc_textarea() and bc_native_select(), are plain native elements that Shiny
# already reads on its own.
#
#   source(system.file("examples", "shiny-app.R", package = "basecoat"))
